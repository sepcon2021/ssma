...
class InspeccionAlmacenModel extends Model{
    ...
    public function insert($object){
        ...
        try {
            ...
            $query = $conexion_bbdd->prepare('INSERT INTO inspeccion_almacen (id_tipo_inspeccion,sede,id_area,lugar_inspeccion,dni_inspeccionado,dni_responsable,observacion_uno,observacion_dos,observacion_tres,fecha)
			                                        VALUES (:id_tipo_inspeccion,:sede,:id_area,:lugar_inspeccion,:dni_inspeccionado,:dni_responsable,:observacion_uno,:observacion_dos,:observacion_tres,:fecha)');
    ...
    public function insertDetalle($listaDetalle,$id)
    {
        try {
            ...
            $query = $conexion_bbdd->prepare('INSERT INTO inspeccion_almacen_detalle(
                id_inspeccion_almacen,
                respuesta,
                condicion,
                calificacion,
                accion_correctiva,
                dni_responsable,
                fecha_cumplimiento,
                seguimiento,id_tipo_inspeccion_almacen,evidencia)
            VALUES (:id_inspeccion_almacen,:respuesta,:condicion,:calificacion,:accion_correctiva,:dni_responsable,:fecha_cumplimiento,:seguimiento,:id_tipo_inspeccion_almacen,:evidencia)');

            $index = 1;

            foreach($listaDetalle as $detalle) {
                
                $query->bindParam(':id_inspeccion_almacen', $id , PDO::PARAM_INT);
                ...
                $query->bindParam(':id_tipo_inspeccion_almacen',$index, PDO::PARAM_INT);
