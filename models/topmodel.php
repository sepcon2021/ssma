...
class Topmodel extends Model
{
  ...
  public function pruebaBulk($listEvaluado, $listEvaluador)
  {
    ...
    foreach ($listEvaluado as $evaluado) {
      ...
      foreach ($listEvaluador as $evaluador) {
        $queryInfo =  array("evaluador" => $evaluador, "evaluado" => $evaluado);
        array_push($listGeneral, $queryInfo);
      }
    }

    try {
      $query = $this->db->connect()->prepare('INSERT INTO ABC (dni1,dni2)
                                        VALUES (:dni1,:dni2)');
      $query->execute([
        'dni1' => "123",
        'dni2' => "789",
